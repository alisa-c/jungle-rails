...
RSpec.describe User, :type => :model do
  ...
  describe 'Validations' do
    ...
    before do
      @user = User.create(first_name: "Anything", last_name: "Anything",
                          email: "help@test.com", password: "test123", password_confirmation: "test123")
    end
  ...
  describe '.authenticate_with_credentials' do
    it "is valid if password matches db" do
      expect(User.authenticate_with_credentials('test@test.com', 'test123')).to be_truthy
    end

    it "is valid if email uppercase" do
      expect(User.authenticate_with_credentials('TEST@test.com', 'test123')).to be_truthy
    end

    it "is valid if email has space" do
      expect(User.authenticate_with_credentials(' test@test.com ', 'test123')).to be_truthy
    end
  end
end